...
from boa_types.tree_node import TreeNode, NodeStatus, NodeRole, collect_last_assistant_text
...
from components.pattern_scorer import analyze_patterns, compute_pattern_boosted_score, pre_judge
from components.prompt_classifier import classify_prompt, PromptCluster
...
class Executor:
    ...
    def run(
        ...
        while True:
            ...
            # phase_aware: more samples for shallow nodes
            if (hasattr(self.searcher, 'min_depth')
                    and node.depth < self.searcher.min_depth):
                target_count = self.config.pa_shallow_samples
            for child in new_nodes:
                ...
                if self._enable_sampling_cache:
                    ...
                    child.scores.extend(cache_result.cached_scores)
                ...
                if needed_count > 0:
                    ...
                    child.score = float(sum(child.scores) / len(child.scores)) if child.scores else 250.0
                    child.selection_score = self._compute_selection_score(child.scores, node=child)
    ...
    def _finalize_rollout_node_if_ready(self, node: TreeNode, source: str) -> bool:
        ...
        node.score = float(sum(scores) / len(scores)) if scores else 250.0
        node.selection_score = self._compute_selection_score(scores, node=node)
    ...
    def _compute_selection_score(self, scores: list[float], node: Optional[TreeNode] = None) -> float:
        if not scores:
            return 250.0
        mean_s = sum(scores) / len(scores)
        if self.config.search_strategy in ("greedy", "mcts"):
            base_score = mean_s
        else:
            alpha = self.config.search_alpha
            sorted_desc = sorted(scores, reverse=True)
            top2_mean = sum(sorted_desc[:2]) / min(2, len(sorted_desc))
            base_score = (1 - alpha) * mean_s + alpha * top2_mean

        # --- Optimization #1: Pattern-aware scoring ---
        if node is not None and self.config.enable_pattern_scoring:
            node_text = collect_last_assistant_text(node)
            pr = analyze_patterns(node_text)
            node.metadata["pattern_stage"] = pr.stage
            node.metadata["pattern_score"] = pr.pattern_score
            if pr.pattern_score > 0:
                base_score = compute_pattern_boosted_score(
                    base_score,
                    pr.pattern_score,
                    alpha=self.config.pattern_score_alpha,
                    beta=self.config.pattern_score_beta,
                )

            # --- Optimization #2: Early potential detection ---
            if self.config.enable_early_potential_detection:
                if pr.is_very_high_potential:
                    base_score += self.config.very_high_potential_bonus
                    node.metadata["high_potential"] = "very_high"
                elif pr.is_high_potential:
                    base_score += self.config.high_potential_bonus
                    node.metadata["high_potential"] = "high"
