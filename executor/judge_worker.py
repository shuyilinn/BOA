...
from boa_types.tree_node import collect_last_assistant_text
...
class JudgeWorker:
    ...
    def flush_once(
        ...
        def _run_chunk(chunk_tasks: List[BufferItem]) -> List[Dict[str, Any]]:
            ...
            for i, task in enumerate(chunk_tasks):
                metadatas[i]["_seq_text"] = collect_last_assistant_text(task.node, self.sampler.tokenizer) + (task.seq_text or "")
    ...
    def _expanding_and_judging(self, tasks: List[BufferItem]) -> Tuple[List[Dict[str, Any]], int]:
        ...
        def _run_expand_chunk(chunk_tasks: List[BufferItem]) -> Tuple[List[Dict[str, Any]], int]:
            ...
            for i, task in enumerate(chunk_tasks):
                chunk_metadatas[i]["_seq_text"] = collect_last_assistant_text(task.node, self.sampler.tokenizer) + (task.seq_text or "")
