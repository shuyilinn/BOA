...
from boa_types.tree_node import NodeRole, collect_last_assistant_text
...
class AgentSafetyBenchEnvironment(BaseEnvironment):
    ...
    def run(self, request: EnvironmentRequestBundle) -> EnvironmentFeedbackBundle:
        ...
        llm_text = collect_last_assistant_text(assistant_node, self._tokenizer)
        ...
        # here ensures that node.text / get_path_text() stay consistent with the
        # semantic content rather than leaking Qwen-specific template tokens into the
        # tree path.  See Fix #3 in conversation_state refactor.
