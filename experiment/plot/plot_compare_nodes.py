...
# -*- coding: utf-8 -*-
"""
Compare total nodes across different settings

Usage:
    python plot_compare_nodes.py
    
Configuration:
    Edit SETTINGS_FOLDERS to define which settings to compare
    
Output:
    - Comparison chart saved in ./figures_mlsys/ as nodes_comparison.pdf
"""
...
def get_total_nodes_over_time(folder_path):
    """Get total nodes over time using time snapshots"""
    ...
    # For each time point, calculate total nodes
    times = [0]
    total_nodes_list = [0]
    ...
    for time_seconds, filepath in time_data:
        ...
        for result_data in all_results:
            ...
            node_count = len(result_data['nodes'])
            prompt_history[result_num] = {
                'nodes': node_count,
                'last_seen_time': time_seconds
            }
        
        # Calculate total nodes from last-seen states of all completed prompts
        total_nodes = 0
        for result_num, info in result_info.items():
            ...
            if info['running_time'] is not None and info['running_time'] <= time_seconds:
                if result_num in prompt_history:
                    total_nodes += prompt_history[result_num]['nodes']
        
        times.append(time_seconds)
        total_nodes_list.append(total_nodes)
    
    return times, total_nodes_list

def compare_nodes():
    """Create a comparison plot of total nodes for different settings"""
    print("="*80)
    print("Total Nodes Comparison - Over Time")
    print("="*80)
    ...
    for setting_name, folder_path in SETTINGS_FOLDERS.items():
        ...
        times, total_nodes = get_total_nodes_over_time(folder_path)
        
        if times is None or total_nodes is None:
        ...
        plt.plot(times, total_nodes, '-', linewidth=2.5, label=setting_name, color=color)
        print(f"  Final total nodes: {total_nodes[-1]} at {times[-1]}s")
    ...
    output_path = os.path.join(output_dir, 'nodes_comparison.pdf')
...
def main():
    """Main function - compare total nodes for all settings"""
    compare_nodes()
