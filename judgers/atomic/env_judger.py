...
class EnvJudger(BaseAtomicJudger):
    ...
    def batch_judge(self, samples: List[JudgerSample]) -> List[AtomicJudgerResult]:
        ...
        for sample in samples:
            full_response = sample.metadata.get("_full_response", "")
            logger.info("EnvJudger: full_response(len=%d)=%.300r", len(full_response), full_response)
            tool_call = _parse_tool_call(full_response) if full_response else None
            if tool_call is None:
                logger.info(
                    "EnvJudger CONTINUE: no tool call found. full_response(len=%d)=%.300r",
